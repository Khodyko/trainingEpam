...
public abstract class MyFilter {
	...
	public void setActivate(boolean activate) { //I think, it isn't needed to override this method
		...
		if(activate){
			controller.showMessage("Activating "+ this.getClass().getSimpleName().replace("Filter"," Filter")+"...");

			getParamForActivation();//Exception!!!!!!!!!???
			// if min>max??????
			this.isActivate=activate;
			controller.showMessage( this.getClass().getSimpleName().replace("Filter"," Filter")+" successfully activated!");}
		else{
			controller.showMessage("Deactivating "+ this.getClass().getSimpleName().replace("Filter"," Filter")+"...");

			this.isActivate=activate;
			controller.showMessage(this.getClass().getSimpleName().replace("Filter"," Filter")+"successfully Deactivated!");
		}
	...
	public abstract void  getParamForActivation(); // It's very important to put there setting of parameters that used in Filtering, when override
	}
